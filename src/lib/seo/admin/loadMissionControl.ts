// src/lib/seo/admin/loadMissionControl.ts
// ============================================================================
// Server-only data loader for /admin/seo.
//
// Assembles the MissionControlPayload the client renders.
//
// Read strategy — RPC-first, paged fallback:
//   Every heavy aggregation (daily trend, page-type breakdown, top pages,
//   visibility counts) is served by a small compact-result RPC defined in
//   migrations/2026-09-20-seo-admin-dashboard-rpcs.sql. If any RPC is
//   missing (migration not yet applied on this environment), that section
//   falls back to the pre-migration paged-read + in-Node aggregation path
//   so the dashboard keeps working. Once the migration is applied, the
//   next revalidate hit picks up the fast path automatically.
//
// Cold render benchmarks against the live DB (measured 2026-09-20):
//     paged fallback :  ~57 seconds  ·  ~329 requests  ·  ~322k rows
//     RPC fast path  :   <3 seconds  ·      ~8 requests ·     ~55 rows
// ============================================================================

import 'server-only'
import { getSupabaseServiceClient } from '@/lib/supabaseService'
import { canonicaliseUrl } from '@/lib/seo/canonicaliseUrl'
import type {
  MissionControlPayload, LatestKpi, DailyPoint,
  MomentumBlock, PageTypeRow, TopPageRow, PageTypeKey, PageTypeBucketKey,
  FunnelBlock, VisibilityBlock, DataHealthBlock, ReconciliationBlock,
} from './types'

const SITE_KEY = 'pokeprices'
const SOURCE   = 'google'
const TARGET_DATE = '2026-12-25'
const TARGET_MIN  = 4000
const TARGET_MAX  = 5000
const BQ_EXPORT_STARTED_ON = '2026-09-16'   // annotation for the chart

// ── date helpers ─────────────────────────────────────────────────────────
function isoDay(d: Date | string): string {
  if (typeof d === 'string') return d.slice(0, 10)
  const yr = d.getUTCFullYear()
  const mo = String(d.getUTCMonth() + 1).padStart(2, '0')
  const dy = String(d.getUTCDate()).padStart(2, '0')
  return `${yr}-${mo}-${dy}`
}
function addDaysISO(iso: string, n: number): string {
  const [y, m, d] = iso.split('-').map(Number)
  const t = Date.UTC(y, (m ?? 1) - 1, d ?? 1) + n * 86400_000
  return isoDay(new Date(t))
}
function daysBetween(fromISO: string, toISO: string): number {
  const [y1, m1, d1] = fromISO.split('-').map(Number)
  const [y2, m2, d2] = toISO.split('-').map(Number)
  return Math.round((Date.UTC(y2, m2 - 1, d2) - Date.UTC(y1, m1 - 1, d1)) / 86400_000)
}

// ── RPC helper ───────────────────────────────────────────────────────────
type SupaError = { message: string; code?: string } | null

function isFunctionMissing(err: SupaError): boolean {
  if (!err) return false
  if (err.code === 'PGRST202' || err.code === 'PGRST203') return true
  const m = err.message || ''
  return m.includes('Could not find the function') ||
         m.includes('does not exist') && m.includes('function')
}

/** Call a Postgres RPC. Returns `{ ok: true, data }` on success,
 *  `{ ok: false, missing: true }` if the function does not exist,
 *  or throws on any other error. */
async function callRpc<T>(name: string, args: Record<string, unknown>): Promise<
  { ok: true; data: T } | { ok: false; missing: true }
> {
  const supa = getSupabaseServiceClient()
  const { data, error } = await supa.rpc(name, args)
  if (error) {
    if (isFunctionMissing(error)) return { ok: false, missing: true }
    throw new Error(`rpc ${name}: ${error.message}`)
  }
  return { ok: true, data: data as T }
}

// ── generic paged reader (fallback path) ─────────────────────────────────
async function pagedRead<Row>(
  builder: (offset: number, chunk: number) => Promise<{ data: Row[] | null; error: SupaError }>,
  chunk = 1000,
): Promise<Row[]> {
  const rows: Row[] = []
  let offset = 0
  while (true) {
    const { data, error } = await builder(offset, chunk)
    if (error) throw new Error(`pagedRead failed at offset ${offset}: ${error.message}`)
    if (!data || data.length === 0) break
    rows.push(...data)
    if (data.length < chunk) break
    offset += chunk
  }
  return rows
}

// ── section loaders: small/fast (always) ─────────────────────────────────

async function loadLatestKpi(): Promise<LatestKpi | null> {
  const supa = getSupabaseServiceClient()
  const { data, error } = await supa
    .from('seo_kpi_daily')
    .select('*')
    .eq('site_key', SITE_KEY)
    .eq('source', SOURCE)
    .order('date', { ascending: false })
    .limit(1)
    .maybeSingle()
  if (error) throw new Error(`loadLatestKpi: ${error.message}`)
  if (!data) return null
  const clicks = Number(data.clicks_28d ?? 0)
  const impr = Number(data.impressions_28d ?? 0)
  const sp = data.sum_position_28d == null ? null : Number(data.sum_position_28d)
  return {
    as_of_date: isoDay(data.date),
    clicks_28d: clicks,
    impressions_28d: impr,
    pages_with_impressions_28d: Number(data.pages_with_impressions_28d ?? 0),
    pages_ge1_click_28d:  Number(data.pages_ge1_click_28d  ?? 0),
    pages_ge10_click_28d: Number(data.pages_ge10_click_28d ?? 0),
    pages_ge28_click_28d: Number(data.pages_ge28_click_28d ?? 0),
    total_urls_known: Number(data.total_urls_known ?? 0),
    urls_indexable:   Number(data.urls_indexable   ?? 0),
    urls_in_sitemap:  Number(data.urls_in_sitemap  ?? 0),
    sum_position_28d: sp,
    ctr_28d:          impr > 0 ? clicks / impr : null,
    avg_position_28d: (sp != null && impr > 0) ? (sp / impr) + 1 : null,
    refreshed_at: (data.refreshed_at as string | null) ?? null,
  }
}

async function loadLatestGscDate(): Promise<{ date: string | null; source: 'google' | null }> {
  const supa = getSupabaseServiceClient()
  const { data, error } = await supa
    .from('seo_gsc_page_daily')
    .select('date')
    .eq('site_key', SITE_KEY)
    .eq('source', SOURCE)
    .order('date', { ascending: false })
    .limit(1)
    .maybeSingle()
  if (error) throw new Error(`loadLatestGscDate: ${error.message}`)
  return { date: data ? isoDay(data.date) : null, source: data ? 'google' : null }
}

async function loadRegistryLastSeenMax(): Promise<string | null> {
  const supa = getSupabaseServiceClient()
  const { data, error } = await supa
    .from('seo_pages')
    .select('last_seen_at')
    .eq('site_key', SITE_KEY)
    .order('last_seen_at', { ascending: false })
    .limit(1)
    .maybeSingle()
  if (error) throw new Error(`loadRegistryLastSeenMax: ${error.message}`)
  return (data?.last_seen_at as string | undefined) ?? null
}

async function loadDataHealth(
  kpi: LatestKpi | null, latestGscDate: string | null, registrySize: number,
): Promise<DataHealthBlock> {
  const supa = getSupabaseServiceClient()
  const registry_last_seen_max = await loadRegistryLastSeenMax()

  const { data: lastRun, error: e1 } = await supa
    .from('seo_bq_ingest_runs')
    .select('started_at, ended_at, job_kind, source, status')
    .eq('site_key', SITE_KEY)
    .eq('status', 'ok')
    .order('started_at', { ascending: false })
    .limit(1)
    .maybeSingle()
  if (e1) throw new Error(`data health (last ok): ${e1.message}`)

  const since = new Date(Date.now() - 14 * 86400_000).toISOString()
  const { data: fails, error: e2 } = await supa
    .from('seo_bq_ingest_runs')
    .select('started_at, ended_at, job_kind, source, error, status')
    .eq('site_key', SITE_KEY)
    .in('status', ['error', 'aborted_budget'])
    .gte('started_at', since)
    .order('started_at', { ascending: false })
    .limit(10)
  if (e2) throw new Error(`data health (failures): ${e2.message}`)

  // Most-recent page_daily_ingest run (any status).
  const { data: lastPageDaily, error: e3 } = await supa
    .from('seo_bq_ingest_runs')
    .select('started_at, ended_at, status, rows_ingested, bytes_scanned, estimated_cost_usd, error')
    .eq('site_key', SITE_KEY)
    .eq('job_kind', 'page_daily_ingest')
    .order('started_at', { ascending: false })
    .limit(1).maybeSingle()
  if (e3) throw new Error(`data health (page_daily_ingest): ${e3.message}`)

  // Most-recent rollup_refresh run (any status).
  const { data: lastRefresh, error: e4 } = await supa
    .from('seo_bq_ingest_runs')
    .select('started_at, ended_at, status, error')
    .eq('site_key', SITE_KEY)
    .eq('job_kind', 'rollup_refresh')
    .order('started_at', { ascending: false })
    .limit(1).maybeSingle()
  if (e4) throw new Error(`data health (rollup_refresh): ${e4.message}`)

  return {
    latest_gsc_date: latestGscDate,
    latest_gsc_date_source: latestGscDate ? 'google' : null,
    latest_kpi_date: kpi?.as_of_date ?? null,
    latest_kpi_refreshed_at: kpi?.refreshed_at ?? null,
    latest_ingest_at: (lastRun?.ended_at ?? lastRun?.started_at) as string | null,
    latest_ingest_kind: (lastRun?.job_kind as string | undefined) ?? null,
    latest_ingest_status: (lastRun?.status as string | undefined) ?? null,
    registry_size: registrySize,
    registry_last_seen_max,
    latest_page_daily_run: lastPageDaily ? {
      started_at:  lastPageDaily.started_at as string,
      ended_at:    (lastPageDaily.ended_at as string | null) ?? null,
      status:      lastPageDaily.status as string,
      rows_ingested: (lastPageDaily.rows_ingested as number | null) ?? null,
      bytes_scanned: (lastPageDaily.bytes_scanned as number | null) ?? null,
      estimated_cost_usd: (lastPageDaily.estimated_cost_usd as number | null) ?? null,
      error:       (lastPageDaily.error as string | null) ?? null,
    } : null,
    latest_rollup_refresh_run: lastRefresh ? {
      started_at:  lastRefresh.started_at as string,
      ended_at:    (lastRefresh.ended_at as string | null) ?? null,
      status:      lastRefresh.status as string,
      error:       (lastRefresh.error as string | null) ?? null,
    } : null,
    recent_failures: (fails ?? []).map(r => ({
      started_at: r.started_at as string,
      ended_at:   (r.ended_at as string | null) ?? null,
      job_kind:   r.job_kind as string,
      source:     r.source as string,
      error:      (r.error as string | null) ?? null,
    })),
  }
}

// ── section loaders: heavy (RPC-first, paged fallback) ───────────────────

/** Fast path: seo_admin_daily_totals RPC returns ~30 rows. */
async function loadDailyTrend(): Promise<DailyPoint[]> {
  const rpc = await callRpc<Array<{
    d: string; impressions: number; clicks: number; sum_position: number
  }>>('seo_admin_daily_totals', {
    p_site_key: SITE_KEY,
    p_source: SOURCE,
    p_start: null,
    p_end: null,
  })
  if (rpc.ok) {
    return rpc.data.map(r => {
      const imp = Number(r.impressions ?? 0)
      const clk = Number(r.clicks ?? 0)
      const sp  = Number(r.sum_position ?? 0)
      return {
        date: isoDay(r.d),
        clicks: clk,
        impressions: imp,
        ctr: imp > 0 ? clk / imp : null,
        avg_position: imp > 0 ? (sp / imp) + 1 : null,
      }
    })
  }
  // Fallback: paged read + Node aggregation.
  return loadDailyTrendPaged()
}

async function loadDailyTrendPaged(): Promise<DailyPoint[]> {
  const supa = getSupabaseServiceClient()
  const rows = await pagedRead<{ date: string; url: string; impressions: number; clicks: number; sum_position: number }>(
    async (offset, chunk) => {
      const res = await supa
        .from('seo_gsc_page_daily')
        .select('date, url, impressions, clicks, sum_position')
        .eq('site_key', SITE_KEY)
        .eq('source', SOURCE)
        .order('date', { ascending: true })
        .order('url', { ascending: true })
        .range(offset, offset + chunk - 1)
      return { data: res.data as any, error: res.error }
    },
  )
  const byDate = new Map<string, { imp: number; clk: number; sp: number }>()
  for (const r of rows) {
    const key = isoDay(r.date)
    const acc = byDate.get(key) ?? { imp: 0, clk: 0, sp: 0 }
    acc.imp += Number(r.impressions || 0)
    acc.clk += Number(r.clicks || 0)
    acc.sp  += Number(r.sum_position || 0)
    byDate.set(key, acc)
  }
  return Array.from(byDate.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([date, v]) => ({
      date, clicks: v.clk, impressions: v.imp,
      ctr: v.imp > 0 ? v.clk / v.imp : null,
      avg_position: v.imp > 0 ? (v.sp / v.imp) + 1 : null,
    }))
}

/** Fast path: seo_admin_page_type_28d RPC. */
type PageTypeRpcRow = {
  page_type: string
  urls_known: number
  urls_visible_28d: number
  clicks_28d: number
  impressions_28d: number
  sum_position_28d: number
  productive_28d: number
}

async function loadPageTypes(): Promise<PageTypeRow[] | null> {
  const rpc = await callRpc<PageTypeRpcRow[]>('seo_admin_page_type_28d', {
    p_site_key: SITE_KEY,
    p_source: SOURCE,
  })
  if (!rpc.ok) return null
  return rpc.data.map(r => {
    const imp = Number(r.impressions_28d)
    const clk = Number(r.clicks_28d)
    const sp  = Number(r.sum_position_28d)
    return {
      page_type: r.page_type as PageTypeBucketKey,
      urls_known: Number(r.urls_known),
      urls_visible_28d: Number(r.urls_visible_28d),
      clicks_28d: clk,
      impressions_28d: imp,
      sum_position_28d: sp,
      productive_28d: Number(r.productive_28d),
      ctr_28d: imp > 0 ? clk / imp : null,
      avg_position_28d: imp > 0 ? sp / imp + 1 : null,
    }
  })
}

/** Fast path: seo_admin_top_pages RPC. */
type TopPageRpcRow = {
  url: string
  page_type: string
  entity_id: string | null
  raw_variant_count: number
  clicks_28d: number
  impressions_28d: number
  sum_position_28d: number
  productive_28d: boolean
}

async function loadTopPages(): Promise<TopPageRow[] | null> {
  const rpc = await callRpc<TopPageRpcRow[]>('seo_admin_top_pages', {
    p_site_key: SITE_KEY,
    p_source: SOURCE,
    p_limit: 20,
  })
  if (!rpc.ok) return null
  return rpc.data.map(r => {
    const imp = Number(r.impressions_28d)
    const clk = Number(r.clicks_28d)
    const sp  = Number(r.sum_position_28d)
    return {
      url: r.url,
      raw_urls: [],  // RPC doesn't return raw URL list; UI shows count only
      page_type: r.page_type as PageTypeBucketKey,
      entity_id: r.entity_id,
      clicks_28d: clk,
      impressions_28d: imp,
      sum_position_28d: sp,
      ctr_28d: imp > 0 ? clk / imp : null,
      avg_position_28d: imp > 0 ? sp / imp + 1 : null,
      productive_28d: !!r.productive_28d,
    }
  })
}

/** Fast path: seo_admin_visibility_28d RPC returns a single row with
 *  every count the visibility + reconciliation panels need. */
type VisibilityRpcRow = {
  known_urls: number
  sitemap_urls: number
  canonical_zero_visibility: number
  sitemap_zero_visibility: number
  canonical_visible_pages: number
  canonical_visible_no_clicks: number
  canonical_ge1_click: number
  canonical_ge10_click: number
  canonical_productive: number
  rollup_row_count: number
  canonical_url_count: number
  canonicalisation_failures: number
  rollup_clicks_28d: number
  rollup_impressions_28d: number
  rollup_visible_28d: number
  rollup_productive_28d: number
  page_lookup_fails: number
  unmatched_visible_28d: number
  unmatched_clicks_28d: number
  unmatched_impressions_28d: number
}

async function loadVisibilitySnapshot(): Promise<VisibilityRpcRow | null> {
  const rpc = await callRpc<VisibilityRpcRow[]>('seo_admin_visibility_28d', {
    p_site_key: SITE_KEY,
    p_source: SOURCE,
  })
  if (!rpc.ok) return null
  const row = Array.isArray(rpc.data) ? rpc.data[0] : (rpc.data as any)
  if (!row) return null
  return row as VisibilityRpcRow
}

// ── slow-path fallback: original paged aggregation ───────────────────────
// Kept intact so /admin/seo continues rendering even if the migration
// has not been applied yet. Once the migration lands, RPC results
// override every one of these branches.

type PageRow    = { url: string; page_type: string; entity_id: string | null }
type RollupRow  = {
  url: string
  clicks_7d: number
  clicks_28d: number
  impressions_7d: number
  impressions_28d: number
  sum_position_28d: number | null
  productive_28d: boolean
}
type CanonicalRollup = {
  canonical_url: string
  raw_urls: string[]
  clicks_7d: number
  clicks_28d: number
  impressions_7d: number
  impressions_28d: number
  sum_position_28d: number
  productive_28d: boolean
}

async function loadPagesMapPaged(): Promise<Map<string, PageRow>> {
  const supa = getSupabaseServiceClient()
  const rows = await pagedRead<PageRow>(async (offset, chunk) => {
    const res = await supa
      .from('seo_pages')
      .select('url, page_type, entity_id')
      .eq('site_key', SITE_KEY)
      .order('url', { ascending: true })
      .range(offset, offset + chunk - 1)
    return { data: res.data as any, error: res.error }
  })
  const m = new Map<string, PageRow>()
  for (const r of rows) m.set(r.url, r)
  return m
}

async function loadRollupsPaged(): Promise<RollupRow[]> {
  const supa = getSupabaseServiceClient()
  return pagedRead<RollupRow>(async (offset, chunk) => {
    const res = await supa
      .from('seo_page_rollups')
      .select('url, clicks_7d, clicks_28d, impressions_7d, impressions_28d, sum_position_28d, productive_28d')
      .eq('site_key', SITE_KEY)
      .eq('source', SOURCE)
      .order('url', { ascending: true })
      .range(offset, offset + chunk - 1)
    return { data: res.data as any, error: res.error }
  })
}

async function loadInSitemapFlagsPaged(): Promise<Map<string, boolean>> {
  const supa = getSupabaseServiceClient()
  const rows = await pagedRead<{ url: string; in_sitemap: boolean }>(async (offset, chunk) => {
    const res = await supa
      .from('seo_pages')
      .select('url, in_sitemap')
      .eq('site_key', SITE_KEY)
      .order('url', { ascending: true })
      .range(offset, offset + chunk - 1)
    return { data: res.data as any, error: res.error }
  })
  const m = new Map<string, boolean>()
  for (const r of rows) m.set(r.url, !!r.in_sitemap)
  return m
}

function canonicalisRollupsInNode(rollups: RollupRow[]): Map<string, CanonicalRollup> {
  const canonical = new Map<string, CanonicalRollup>()
  for (const r of rollups) {
    const c = canonicaliseUrl(r.url) ?? r.url
    const entry = canonical.get(c) ?? {
      canonical_url: c, raw_urls: [],
      clicks_7d: 0, clicks_28d: 0,
      impressions_7d: 0, impressions_28d: 0,
      sum_position_28d: 0, productive_28d: false,
    }
    entry.raw_urls.push(r.url)
    entry.clicks_7d       += Number(r.clicks_7d ?? 0)
    entry.clicks_28d      += Number(r.clicks_28d ?? 0)
    entry.impressions_7d  += Number(r.impressions_7d ?? 0)
    entry.impressions_28d += Number(r.impressions_28d ?? 0)
    entry.sum_position_28d += Number(r.sum_position_28d ?? 0)
    canonical.set(c, entry)
  }
  canonical.forEach(e => { e.productive_28d = e.clicks_28d >= 28 })
  return canonical
}

function slowPathAggregation(
  pagesByUrl: Map<string, PageRow>,
  canonicalRollups: Map<string, CanonicalRollup>,
  inSitemapFlags: Map<string, boolean>,
) {
  const perType = new Map<PageTypeBucketKey, PageTypeRow>()
  const knownByType = new Map<PageTypeKey, number>()
  pagesByUrl.forEach(p => {
    const pt = p.page_type as PageTypeKey
    knownByType.set(pt, (knownByType.get(pt) ?? 0) + 1)
  })
  const unmatched = { visible: 0, clicks: 0, impressions: 0, page_lookup_fails: 0 }
  const emptyRow = (page_type: PageTypeBucketKey, urls_known: number): PageTypeRow => ({
    page_type, urls_known, urls_visible_28d: 0,
    clicks_28d: 0, impressions_28d: 0, sum_position_28d: 0,
    productive_28d: 0, ctr_28d: null, avg_position_28d: null,
  })
  canonicalRollups.forEach(entry => {
    const page = pagesByUrl.get(entry.canonical_url)
    const pt: PageTypeBucketKey = page ? (page.page_type as PageTypeKey) : 'unmatched'
    if (!page) {
      unmatched.page_lookup_fails++
      unmatched.clicks      += entry.clicks_28d
      unmatched.impressions += entry.impressions_28d
      if (entry.impressions_28d > 0) unmatched.visible++
    }
    const acc = perType.get(pt) ?? emptyRow(pt, pt === 'unmatched' ? 0 : (knownByType.get(pt as PageTypeKey) ?? 0))
    if (entry.impressions_28d > 0) acc.urls_visible_28d++
    acc.clicks_28d       += entry.clicks_28d
    acc.impressions_28d  += entry.impressions_28d
    acc.sum_position_28d += entry.sum_position_28d
    if (entry.productive_28d) acc.productive_28d++
    perType.set(pt, acc)
  })
  knownByType.forEach((known, pt) => {
    if (!perType.has(pt)) perType.set(pt, emptyRow(pt, known))
  })
  const page_types: PageTypeRow[] = Array.from(perType.values())
    .map(a => ({
      ...a,
      ctr_28d: a.impressions_28d > 0 ? a.clicks_28d / a.impressions_28d : null,
      avg_position_28d: a.impressions_28d > 0 ? (a.sum_position_28d / a.impressions_28d) + 1 : null,
    }))
    .sort((a, b) => b.clicks_28d - a.clicks_28d || b.impressions_28d - a.impressions_28d)

  const top_pages: TopPageRow[] = Array.from(canonicalRollups.values())
    .sort((a, b) => b.clicks_28d - a.clicks_28d || b.impressions_28d - a.impressions_28d)
    .slice(0, 20)
    .map(entry => {
      const page = pagesByUrl.get(entry.canonical_url)
      const pt: PageTypeBucketKey = page ? (page.page_type as PageTypeKey) : 'unmatched'
      return {
        url: entry.canonical_url,
        raw_urls: entry.raw_urls,
        page_type: pt,
        entity_id: page?.entity_id ?? null,
        clicks_28d: entry.clicks_28d,
        impressions_28d: entry.impressions_28d,
        sum_position_28d: entry.sum_position_28d,
        ctr_28d: entry.impressions_28d > 0 ? entry.clicks_28d / entry.impressions_28d : null,
        avg_position_28d: entry.impressions_28d > 0 ? entry.sum_position_28d / entry.impressions_28d + 1 : null,
        productive_28d: entry.productive_28d,
      }
    })

  let known_zero_visibility = 0
  let sitemap_zero_visibility = 0
  let visible_no_clicks = 0
  let visible_ge1_click = 0
  let visible_productive = 0
  pagesByUrl.forEach(p => {
    const r = canonicalRollups.get(p.url)
    const impressions_28d = r ? r.impressions_28d : 0
    const clicks_28d      = r ? r.clicks_28d      : 0
    const in_sitemap      = !!inSitemapFlags.get(p.url)
    if (impressions_28d === 0) {
      known_zero_visibility++
      if (in_sitemap) sitemap_zero_visibility++
    } else {
      if (clicks_28d === 0) visible_no_clicks++
      if (clicks_28d >= 1)  visible_ge1_click++
      if (clicks_28d >= 28) visible_productive++
    }
  })
  const visibility: VisibilityBlock = {
    known_zero_visibility,
    sitemap_zero_visibility,
    visible_no_clicks, visible_ge1_click, visible_productive,
  }

  let rollup_clicks_28d = 0, rollup_impressions_28d = 0
  let rollup_visible_28d = 0, rollup_productive_28d = 0
  canonicalRollups.forEach(e => {
    rollup_clicks_28d      += e.clicks_28d
    rollup_impressions_28d += e.impressions_28d
    if (e.impressions_28d > 0) rollup_visible_28d++
    if (e.productive_28d) rollup_productive_28d++
  })

  return {
    page_types, top_pages, visibility, unmatched,
    rollup_rows_loaded: 0,   // filled by caller
    rollup_canonical_urls: canonicalRollups.size,
    rollup_clicks_28d, rollup_impressions_28d,
    rollup_visible_28d, rollup_productive_28d,
    matched_pages_lookup: canonicalRollups.size - unmatched.page_lookup_fails,
    canonicalisation_failures: 0,
    registry_size: pagesByUrl.size,
  }
}

// ── momentum ─────────────────────────────────────────────────────────────

function computeMomentum(daily: DailyPoint[]): MomentumBlock | null {
  if (daily.length === 0) return null
  const latest = daily[daily.length - 1].date
  const cur_from  = addDaysISO(latest, -6)
  const prev_to   = addDaysISO(latest, -7)
  const prev_from = addDaysISO(latest, -13)
  const sum = (a: string, b: string) => {
    let imp = 0, clk = 0, sp = 0, days = 0
    for (const d of daily) {
      if (d.date >= a && d.date <= b) {
        imp += d.impressions; clk += d.clicks
        if (d.avg_position != null && d.impressions > 0) sp += (d.avg_position - 1) * d.impressions
        days++
      }
    }
    return {
      from: a, to: b, days,
      impressions: imp, clicks: clk, sum_position: sp,
      ctr: imp > 0 ? clk / imp : null,
      avg_position: imp > 0 ? sp / imp + 1 : null,
    }
  }
  const cur = sum(cur_from, latest)
  const prev = sum(prev_from, prev_to)
  const pct = (nowV: number, prevV: number): number | null => {
    if (prevV === 0) return nowV === 0 ? 0 : null
    return (nowV - prevV) / prevV * 100
  }
  return {
    current: cur,
    previous: prev,
    click_pct_change:      pct(cur.clicks, prev.clicks),
    impression_pct_change: pct(cur.impressions, prev.impressions),
    ctr_delta:             (cur.ctr != null && prev.ctr != null) ? (cur.ctr - prev.ctr) : null,
    avg_position_delta:    (cur.avg_position != null && prev.avg_position != null) ? (cur.avg_position - prev.avg_position) : null,
  }
}

// ── main entry point ─────────────────────────────────────────────────────

export async function loadMissionControl(): Promise<MissionControlPayload> {
  // A. Small fast queries always. Parallel.
  const [kpi, latestGscInfo] = await Promise.all([
    loadLatestKpi(),
    loadLatestGscDate(),
  ])
  if (!kpi) throw new Error('loadMissionControl: no seo_kpi_daily row for google/pokeprices')

  // B. Try RPC fast path for the four heavy sections. Parallel.
  const [daily, pageTypeFast, topPagesFast, visibilityFast] = await Promise.all([
    loadDailyTrend(),          // handles its own fallback internally
    loadPageTypes(),
    loadTopPages(),
    loadVisibilitySnapshot(),
  ])

  let page_types: PageTypeRow[]
  let top_pages: TopPageRow[]
  let visibility: VisibilityBlock
  let reconciliation: ReconciliationBlock
  let registrySize: number

  const fastPathOk = pageTypeFast != null && topPagesFast != null && visibilityFast != null
  if (fastPathOk) {
    // Fast path — assemble from RPC results.
    page_types = pageTypeFast!
    top_pages = topPagesFast!
    const v = visibilityFast!
    visibility = {
      known_zero_visibility:  v.canonical_zero_visibility,
      sitemap_zero_visibility: v.sitemap_zero_visibility,
      visible_no_clicks:      v.canonical_visible_no_clicks,
      visible_ge1_click:      v.canonical_ge1_click,
      visible_productive:     v.canonical_productive,
    }
    reconciliation = {
      rollup_clicks_28d:      Number(v.rollup_clicks_28d),
      rollup_impressions_28d: Number(v.rollup_impressions_28d),
      rollup_visible_28d:     Number(v.rollup_visible_28d),
      rollup_productive_28d:  Number(v.rollup_productive_28d),
      rollup_rows_loaded:     Number(v.rollup_row_count),
      rollup_rows_canonicalisation_failed: Number(v.canonicalisation_failures),
      rollup_canonical_urls:  Number(v.canonical_url_count),
      matched_pages_lookup:   Number(v.canonical_url_count) - Number(v.page_lookup_fails),
      unmatched_pages_lookup: Number(v.page_lookup_fails),
      unmatched_visible_28d:  Number(v.unmatched_visible_28d),
      unmatched_clicks_28d:   Number(v.unmatched_clicks_28d),
      unmatched_impressions_28d: Number(v.unmatched_impressions_28d),
      invariants: {
        // Invariants recomputed from the RPC's own sums vs the page_type rows.
        clicks_match_rollup:      sumRow(page_types, r => r.clicks_28d)      === Number(v.rollup_clicks_28d),
        impressions_match_rollup: sumRow(page_types, r => r.impressions_28d) === Number(v.rollup_impressions_28d),
        visible_match_rollup:     sumRow(page_types, r => r.urls_visible_28d) === Number(v.rollup_visible_28d),
        productive_match_rollup:  sumRow(page_types, r => r.productive_28d)   === Number(v.rollup_productive_28d),
      },
      kpi_clicks_28d:      kpi.clicks_28d,
      kpi_impressions_28d: kpi.impressions_28d,
      kpi_visible_28d:     kpi.pages_with_impressions_28d,
      kpi_productive_28d:  kpi.pages_ge28_click_28d,
      kpi_vs_rollup_delta: {
        clicks:      Number(v.rollup_clicks_28d)       - kpi.clicks_28d,
        impressions: Number(v.rollup_impressions_28d)  - kpi.impressions_28d,
        visible:     Number(v.rollup_visible_28d)      - kpi.pages_with_impressions_28d,
        productive:  Number(v.rollup_productive_28d)   - kpi.pages_ge28_click_28d,
      },
    }
    registrySize = Number(v.known_urls)
  } else {
    // Fallback path — legacy paged reads + Node aggregation.
    const [pagesByUrl, rollups, inSitemapFlags] = await Promise.all([
      loadPagesMapPaged(),
      loadRollupsPaged(),
      loadInSitemapFlagsPaged(),
    ])
    const canonicalRollups = canonicalisRollupsInNode(rollups)
    const slow = slowPathAggregation(pagesByUrl, canonicalRollups, inSitemapFlags)
    page_types = slow.page_types
    top_pages = slow.top_pages
    visibility = slow.visibility
    reconciliation = {
      rollup_clicks_28d: slow.rollup_clicks_28d,
      rollup_impressions_28d: slow.rollup_impressions_28d,
      rollup_visible_28d: slow.rollup_visible_28d,
      rollup_productive_28d: slow.rollup_productive_28d,
      rollup_rows_loaded: rollups.length,
      rollup_rows_canonicalisation_failed: slow.canonicalisation_failures,
      rollup_canonical_urls: slow.rollup_canonical_urls,
      matched_pages_lookup: slow.matched_pages_lookup,
      unmatched_pages_lookup: slow.unmatched.page_lookup_fails,
      unmatched_visible_28d: slow.unmatched.visible,
      unmatched_clicks_28d: slow.unmatched.clicks,
      unmatched_impressions_28d: slow.unmatched.impressions,
      invariants: {
        clicks_match_rollup:      sumRow(page_types, r => r.clicks_28d)      === slow.rollup_clicks_28d,
        impressions_match_rollup: sumRow(page_types, r => r.impressions_28d) === slow.rollup_impressions_28d,
        visible_match_rollup:     sumRow(page_types, r => r.urls_visible_28d) === slow.rollup_visible_28d,
        productive_match_rollup:  sumRow(page_types, r => r.productive_28d)   === slow.rollup_productive_28d,
      },
      kpi_clicks_28d:      kpi.clicks_28d,
      kpi_impressions_28d: kpi.impressions_28d,
      kpi_visible_28d:     kpi.pages_with_impressions_28d,
      kpi_productive_28d:  kpi.pages_ge28_click_28d,
      kpi_vs_rollup_delta: {
        clicks:      slow.rollup_clicks_28d      - kpi.clicks_28d,
        impressions: slow.rollup_impressions_28d - kpi.impressions_28d,
        visible:     slow.rollup_visible_28d     - kpi.pages_with_impressions_28d,
        productive:  slow.rollup_productive_28d  - kpi.pages_ge28_click_28d,
      },
    }
    registrySize = pagesByUrl.size
  }

  const data_health = await loadDataHealth(kpi, latestGscInfo.date, registrySize)
  const momentum = computeMomentum(daily)

  const funnel: FunnelBlock = {
    known:            kpi.total_urls_known,
    in_sitemap:       kpi.urls_in_sitemap,
    visible_28d:      kpi.pages_with_impressions_28d,
    clicks_ge1_28d:   kpi.pages_ge1_click_28d,
    clicks_ge10_28d:  kpi.pages_ge10_click_28d,
    clicks_ge28_28d:  kpi.pages_ge28_click_28d,
  }

  const asOf = kpi.as_of_date
  const nowDate = new Date()
  const today_iso = isoDay(nowDate)
  const generated_at = nowDate.toISOString()
  const days_to_target = Math.max(0, daysBetween(today_iso, TARGET_DATE))
  const newer_gsc_days = (latestGscInfo.date && latestGscInfo.date > asOf)
    ? Math.max(0, daysBetween(asOf, latestGscInfo.date))
    : 0

  return {
    target_date: TARGET_DATE,
    target_clicks_per_day_min: TARGET_MIN,
    target_clicks_per_day_max: TARGET_MAX,
    as_of_date: asOf,
    latest_gsc_date: latestGscInfo.date,
    today_iso,
    generated_at,
    days_to_target,
    newer_gsc_days,
    bq_export_started_on: BQ_EXPORT_STARTED_ON,
    kpi,
    funnel,
    visibility,
    daily,
    momentum,
    page_types,
    top_pages,
    reconciliation,
    data_health,
  }
}

function sumRow<T>(arr: T[], pick: (r: T) => number): number {
  let s = 0
  for (const r of arr) s += pick(r)
  return s
}
