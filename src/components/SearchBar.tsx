...
// "172/199" or "172/165" or "#172" → returns "172"
...
// "Pikachu 172/199" or "Pikachu #172" → { name: "Pikachu", number: "172" }
...
export default function SearchBar({ placeholder = 'Search cards, sets, or number e.g. 172/199…' }: { placeholder?: string }) {
  ...
  useEffect(() => {
    ...
    async function search() {
      ...
      if (pureNumber) {
        ...
        // Name + number: "Pikachu 172"
        const { data: cards } = await supabase
          .from('cards')
          .select('card_slug, card_name, set_name, image_url, card_url_slug, card_number')
          .ilike('card_name', `%${nameAndNumber.name}%`)
          .eq('card_number', nameAndNumber.number)
          .not('card_url_slug', 'is', null)
          .limit(8)
