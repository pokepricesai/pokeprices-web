...
function getGradePrice(card: DealCard, region: Region): number {
  ...
  else if (card.grade === 'cgc95' && card.cgc95Usd) usd = card.cgc95Usd
  else if (card.grade === 'cgc10' && card.cgc10Usd) usd = card.cgc10Usd
...
function SearchBox({ region, onAdd }: { region: Region; onAdd: (card: DealCard) => void }) {
  ...
  async function handleAdd(r: SearchResult) {
    ...
    // price_usd from search_global is in cents — divide by 100
    let psa9Usd: number | null = null
    let psa10Usd: number | null = null
    let cgc95Usd: number | null = null
    let cgc10Usd: number | null = null

    try {
      // Step 1: look up card_slug from cards table using card_url_slug
      ...
      if (cardRow?.card_slug) {
        // Step 2: fetch latest graded prices from daily_prices (also stored in cents)
...
function CardRow({ card, pct, region, valueLabel, valueColor, onRemove, onOverride, onGradeChange }: {
  ...
  return (
    <div style={{ background: 'var(--bg-light)', borderRadius: 12, border: '1px solid var(--border-light)', marginBottom: 8, overflow: 'hidden' }}>
      ...
      {/* Top: image + name + grade pills + remove */}
      ...
      {/* Bottom bar: market price + rate override + offer + eBay link */}
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 9, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, fontFamily: "'Figtree', sans-serif", fontWeight: 700 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {editingPct ? (
              ...
              <button
                onClick={() => { setPctInput(String(effectivePct)); setEditingPct(true) }}
                style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 900, color: valueColor, fontFamily: "'Figtree', sans-serif" }}>
            {fmt(offerPrice, region)}
          </div>
        </div>

        <a
          href={ebayUrl(card, region)}
          target="_blank"
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: '14px' }}>
        ...
        {cards.length === 0 ? (
          ...
          <CardRow
            key={c.id} card={c} pct={pct} region={region}
        ...
        <div style={{ marginTop: 10, padding: '10px 12px', background: 'var(--bg-light)', borderRadius: 10, border: '1px solid var(--border-light)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 16, color: 'var(--text-muted)', fontWeight: 700 }}>{sym(region)}</span>
            <input
        ...
        {(cards.length > 0 || cashVal > 0) && (
          <div style={{ marginTop: 10, padding: '12px 14px', background: 'var(--bg)', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              {cards.length > 0 && <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: "'Figtree', sans-serif", marginTop: 2 }}>Market value: {fmt(totalMarket, region)}</div>}
...
function HotMovers({ region }: { region: Region }) {
    ...
    async function load() {
      const [risersRes, fallersRes] = await Promise.all([
        ...
        supabase.from('card_trends')
          ...
          .lt('raw_pct_30d', 0)
          .gte('raw_pct_30d', -300)
          .order('raw_pct_30d', { ascending: true })
          .limit(10),
      ])
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
      ...
      {rows.length === 0 && (
        <div style={{ padding: '20px', textAlign: 'center', fontSize: 12, color: 'var(--text-muted)', fontFamily: "'Figtree', sans-serif" }}>
          No data yet — check back after the nightly update
        </div>
      )}
      {rows.map((r, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', borderBottom: '1px solid var(--border-light)' }}>
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)', fontFamily: "'Figtree', sans-serif" }}>
              {fmt(convert(r.current_raw / 100, region), region)}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 20 }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px', alignSelf: 'start' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '5px 14px', alignItems: 'center' }}>
            <span />
            <span style={{ fontSize: 10, fontWeight: 700, color: '#16a34a', fontFamily: "'Figtree', sans-serif", textAlign: 'right' }}>Cash</span>
            ...
            {[200, 100, 50, 20, 10, 5].map(v => (
              <>
                <span key={`l${v}`} style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: "'Figtree', sans-serif" }}>{sym(region)}{v}</span>
                <span key={`c${v}`} style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', fontFamily: "'Figtree', sans-serif", textAlign: 'right' }}>{sym(region)}{(v * cashPct / 100).toFixed(0)}</span>
                <span key={`t${v}`} style={{ fontSize: 13, fontWeight: 700, color: 'var(--primary)', fontFamily: "'Figtree', sans-serif", textAlign: 'right' }}>{sym(region)}{(v * tradePct / 100).toFixed(0)}</span>
              </>
            ))}
          </div>
        </div>
