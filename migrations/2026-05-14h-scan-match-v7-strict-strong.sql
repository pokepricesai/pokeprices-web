-- scan_card_match v7: strip dashes from number normalisation + strict-mode
-- filter that shows ONLY strong matches when any exist.
--
-- Charizard promo bug: Vision read "SWSH-123" but DB stored "SWSH123";
-- the prior normaliser kept dashes, so they did not match. Now dashes,
-- underscores and dots are stripped along with whitespace.
--
-- Strict filter: if at least one candidate is a "strong" match — either
--   * confident denominator match (full / with_denom), or
--   * promo scan landing on a promo-set card
-- — then ONLY those strong matches are returned. No name-only fallbacks,
-- no numerator-only noise, no unknown_denom guesses. Confident means
-- confident; do not muddy the list with weaker alternatives.
...
AS $$
  SELECT CASE
    ...
    ELSE regexp_replace(
      -- 1. lowercase, strip whitespace, strip dashes / underscores / dots
      regexp_replace(
        lower(regexp_replace(s, '\s+', '', 'g')),
        '[-_.]', '', 'g'
      ),
      -- 2. strip leading zeros from each numeric run
      '0+([0-9])', '\1', 'g'
    )
  END;
$$;

-- Recreate the expression GIN index so it picks up the new normaliser.
DROP INDEX IF EXISTS idx_cards_norm_card_number;
DROP INDEX IF EXISTS idx_cards_norm_card_number_display;
...
AS $$
  ...
  -- A row is "strong" if it is a confident denominator match OR a promo
  -- scan that landed on a promo-set card. Strong rows pin down a specific
  -- card; weaker rows are speculative.
  scored_strong AS (
    SELECT s.*,
      (
  ...
  filtered AS (
    SELECT s.* FROM scored_strong s, has_strong hs, has_promo_match hp, params p
    WHERE
      -- Strict mode: if any strong match exists, show ONLY strong matches.
      (NOT hs.yes OR s.is_strong)
      -- Promo filter (v6): if scanning a promo and any promo-set match
      -- exists, exclude non-promo candidates.
  ...
  with_base AS (
    SELECT
      ...
      CASE
        ...
        -- Promo-set numerator match (in promo mode): treat as strong.
