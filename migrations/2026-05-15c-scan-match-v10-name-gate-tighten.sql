-- scan_card_match v10: tighten the v9 name gate from 0.10 to 0.30.
--
-- Real bug: Mega Starmie ex 118/088 scan (AI misread number as 113/088)
-- still matched Mega Lucario ex 113/88 from Ascended Heroes because both
-- share "mega" and "ex" which lifts their trigram similarity to ~0.20.
-- v9 used a permissive 0.10 floor so the wrong card slipped through.
--
-- Practical effect at 0.30:
--   - Real partial reads still pass: "Charizard" vs "Charizard ex" ~0.79,
--     "Pikachu" vs "Pikachu V" ~0.75.
--   - Cross-Pokemon false matches now fail: "Mega Lucario ex" vs "Mega
--     Starmie ex" ~0.20, "Charizard" vs "Charmeleon" ~0.21.
--
-- The AI misreading the number itself is not solvable in SQL. If the
-- number is wrong AND no candidate has a plausible name, candidates
-- comes back empty, the UI says "no match", user can retry.
...
AS $$
  ...
  -- Name plausibility gate. Threshold 0.30 is tight enough to reject
  -- shared-prefix collisions ("Mega Lucario" vs "Mega Starmie",
  -- "Charizard" vs "Charmeleon") while leaving legitimate partial reads
  -- like "Charizard" vs "Charizard ex" well above the line.
  scored_namegated AS (
    SELECT s.*,
      (
        p.nm IS NULL
