-- scan_card_match v6: promo-aware matching.
--
-- Promo cards have a "PROMO" badge bottom-left and use a different
-- numbering scheme (SWSH123, XY12, SM01, etc — no set total). When the
-- edge function detects a promo signal from the Vision text, the RPC
-- now filters candidates to sets whose set_name contains "Promo".
--
-- Filter pattern matches the denominator filter from v5: if at least
-- one promo-set candidate exists for this scan, return only those.
-- If none exist (our DB does not have the promo set loaded), fall back
-- to the normal matches so the user still sees something.
...
AS $$
  ...
  filtered AS (
    ...
    --   - exclude unknown_denom if confident-denom matches exist (v5 rule)
    --   - exclude non-promo if is_promo and any promo-set match exists (v6 rule)
    SELECT s.* FROM scored s, has_confident hc, has_promo_match hp, params p
    WHERE (
      s.match_quality IN ('full', 'with_denom', 'numerator', 'name_only')
      OR (s.match_quality = 'unknown_denom' AND NOT hc.yes)
    )
    AND (
      NOT p.is_promo
      OR s.promo_match
